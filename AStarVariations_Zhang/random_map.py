...
class RandomMap:
    def __init__(self):
        ...
        f = open("Test_Grids/Grid_Size,50_Density,0.2.txt", "r")
        map_test = f.read().splitlines()
        self.size = len(map_test) - 2
        for row in range(1, len(map_test) - 1):
            for column in range(1, len(map_test[0]) - 1):
                if map_test[row][column] == "1":
                    self.obstacle_point.append(point.Point(column - 1, self.size - row))
                if map_test[row][column] == "2":
                    self.startX = column - 1
                    self.startY = self.size - row
                if map_test[row][column] == "3":
                    self.endX = column - 1
                    self.endY = self.size - row
        f.close()
